...
public class CColor {
 ...
 private static double COLOR_MATCH_STRENGTH = 98;
 ...
 private static final Map<String, Function<String, String>> COLOR_TRANSLATE_FUNCTIONS = new ConcurrentHashMap<>();
 ...
 public static CColor[] values() {
	return BY_CHAR.values().toArray(new CColor[BY_CHAR.size()]);
 }
 ...
 public static void registerColorTranslator(String name, Function<String, String> colorTranslator) {
	COLOR_TRANSLATE_FUNCTIONS.put(name.toLowerCase(), colorTranslator);
 }

 public static String translateCommon(String text) {
	for(Function<String, String> translateFunction : COLOR_TRANSLATE_FUNCTIONS.values())
	 text = translateFunction.apply(text);
 ...
 public static String translateGradient(String text, GradientCenter center, int steps, int initialStep, CColor... transitions) {
	...
	List<CColor> gradient = CColor.createGradient(center == GradientCenter.LEFT || center == GradientCenter.RIGHT ?
		 steps - (steps / 3) : steps, transitions);
	int increment = 1;
