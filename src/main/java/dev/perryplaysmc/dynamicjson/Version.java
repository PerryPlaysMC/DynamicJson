...
public enum Version {
    ...
    public static class Minecraft {
        public static Class<?> getClass(String clazz) {
            try {
                String nms = Version.isCurrentHigher(v1_16_R3) ? getNMSPackage() : getNMSPackage() + ".server." + getCurrentVersionExact().getVersion();
                if(!Version.isCurrentHigher(v1_16_R3))
                    if(clazz.contains("."))
                        return Class.forName(nms + "." + clazz.split("\\.")[clazz.split("\\.").length-1]);
