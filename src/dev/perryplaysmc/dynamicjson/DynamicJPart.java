...
public class DynamicJPart implements IJsonSerializable {
  ...
  public DynamicJPart setColor(ChatColor color) {
    return setColor(CColor.fromName(color.toString()));
  }
  ...
  public DynamicJPart onHover(DynamicHoverAction action, String value) {
    hoverAction = action;
    hoverData = CColor.translateAlternateColorCodes('&', value);
    return this;
  }
  ...
  public DynamicJPart onHover(Entity enity) {
    return onHover(DynamicHoverAction.SHOW_ENTITY,
           "{\"type\":\"" + enity.getType().name().toLowerCase() + "\",\"id\":" + enity.getUniqueId() + ",\"name\":{\"text\":\"" + enity.getName() + "\"}}");
  }
